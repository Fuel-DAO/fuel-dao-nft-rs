...
use crate::store_state::Chunk;
use crate::store_types::BatchId;
use crate::store_types::ChunkId;
use crate::store_types::CommitBatchArguments;
use crate::store_types::ConfigurationResponse;
use crate::store_types::ConfigureArguments;
use crate::store_types::CreateAssetArguments;
use crate::store_types::CreateChunkArg;
use crate::store_types::DeleteAssetArguments;
use crate::store_types::Permission;
use crate::store_types::SetAssetContentArguments;
use crate::store_types::{AssetProperties, StoreArg};
use crate::store_types::{
    BatchOperation, CommitProposedBatchArguments, ComputeEvidenceArguments, CreateChunksArg,
    DeleteBatchArguments, GetArg, GetChunkArg, SetAssetPropertiesArguments, SetPermissions,
...
impl State {
    ...
    pub fn store(&mut self, arg: StoreArg, time: u64) -> Result<(), String> {
        println!("store function called with parameters:");
        println!("key: {}", arg.key);
        let dependent_keys = self.dependent_keys(&arg.key);
        ...
        println!("Asset stored successfully: {}", arg.key);
        println!(
            "Asset details - Content Type: {}, SHA-256: {:?}",
            asset.content_type, hash
        );
        on_asset_change(&mut self.asset_hashes, &arg.key, asset, dependent_keys);
        Ok(())
    }
    ...
    fn build_http_response(
        ...
        println!("Building HTTP response for path: {}", path);
        ...
        println!("Witness result for path '{}': {:?}", path, witness_result);
        if witness_result == WitnessResult::FallbackFound {
            println!("Fallback asset found for path: {}", path);
            ...
            if let Ok(asset) = self.get_asset(&FALLBACK_FILE.to_string()) {
                if let Some(response) = HttpResponse::build_ok_from_requested_encodings(
                    ...
                    println!(
                        "Built HTTP 200 response with fallback asset for path: {}",
                        path
                    );
            ...
            println!("Asset found for path: {}", path);

            if let Ok(asset) = self.get_asset(&path.into()) {
                if !asset.allow_raw_access() && req.is_raw_domain() {
                    println!("Raw access not allowed for key: {}. Redirecting.", path);
                ...
                if let Some(response) = HttpResponse::build_ok_from_requested_encodings(
                    ...
                    println!("Built HTTP 200 response with asset for path: {}", path);
        ...
        println!(
            "Asset not found and no fallback available for path: {}",
            ath
        );
    ...
    pub fn http_request(
        ...
        println!("Parsed Accept-Encoding headers: {:?}", encodings);
...
fn on_asset_change(
    ...
    println!("Updating certified responses for key: {}", key);
    ...
    println!(
        "Deleted preexisting hashes for affected keys: {:?}",
        affected_keys
    );
    ...
    println!("Updated certificate expressions for key: {}", key);
